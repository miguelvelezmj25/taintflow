...
public class TaintInfoflow extends Infoflow {
  public static final String CONFIG_FILE =
      "src/main/java/edu/cmu/cs/mvelezce/analysis/option/config.json";
...
//    System.out.println("Total assigns: " + assignmentInControlFlowCounter.getNumTotalAssigns());
  ...
  public void aggregateInfoflowResults(int count) throws IOException {
    ...
    Collection<File> files =
        FileUtils.listFiles(
            new File(TaintInfoflow.OUTPUT_DIR + this.systemName), extensions, false);
    ...
    for (File file : files) {
      ...
      List<ControlFlowResult> results =
          mapper.readValue(file, new TypeReference<List<ControlFlowResult>>() {});
    ...
    File outputFile =
        new File(
            TaintInfoflow.OUTPUT_DIR + "/" + this.systemName + "/" + this.systemName + ".json");
  ...
  private void calculateInteractionOrder(List<ControlFlowResult> aggregatedResults) {
    ...
    for (Map.Entry<Integer, Integer> interactionCountToEntryCount :
        interactionCountsToEntryCount.entrySet()) {
      System.out.println(
          "Interaction order= "
              + interactionCountToEntryCount.getKey()
              + " entries= "
              + interactionCountToEntryCount.getValue());
    }
  ...
  public void computeInfoflowOneSourceAtATime(String libPath, String appPath, String entryPoint)
      throws IOException {
    this.computeInfoflowOneSourceAtATime(
        libPath, appPath, entryPoint, this.sourcesToOptions.keySet(), this.sinks);
  }

  public void computeInfoflowOneSourceAtATime(
      String libPath,
      String appPath,
      String entryPoint,
    ...
    for (String source : sources) {
      ...
      System.out.println(
          "############## Option " + currentOption + " results size " + this.getResults().size());
  ...
  private void getSourceToSinkPath(String option) throws IOException {
      ...
      for (ResultSourceInfo source : resultMap.get(sink)) {
        ...
        if (source.getPath() != null) {
          for (Unit p : source.getPath()) {
            ...
            SourceToSinkPath.PathElement element =
                new SourceToSinkPath.PathElement(fullName, javaLine);
            path.add(element);
          }
        }
    ...
    ObjectMapper mapper = new ObjectMapper();
    File outputFile =
        new File(TaintInfoflow.PATHS_DIR + "/" + this.systemName + "/" + option + ".json");
  ...
  private void saveResults(String option) throws IOException {
    ...
    for (ResultSinkInfo resultSinkInfo : resultMap.keySet()) {
      ...
      methodSignature =
          methodSignature.substring(methodSignature.indexOf(":") + 1, methodSignature.length() - 1);
      ...
      for (ResultSourceInfo resultSourceInfo : resultMap.values()) {
        ...
        if (!resultOption.equals(option)) {
          throw new RuntimeException(
              "The option in this source is different than the one we analyzed");
        }
      ...
      ControlFlowResult controlFlowResult =
          new ControlFlowResult(
              packageName, className, methodSignature, bytecodeIndex, javaLine, sources);
    ...
    ObjectMapper mapper = new ObjectMapper();
    File outputFile =
        new File(TaintInfoflow.OUTPUT_DIR + "/" + this.systemName + "/" + option + ".json");
    outputFile.getParentFile().mkdirs();
